#!/usr/bin/env bash
# T9 test mechanism: lazy media facades + media routes.
# Installs the valid fixture set (same collection_for mapping as run-t2),
# builds, serves dist/ via `astro preview` on a fixed port, runs playwright
# against it, then cleans up fixtures and the server. Exits nonzero on any
# failure; playwright prints a per-check PASS line.
set -u
cd "$(dirname "$0")/.."
...
if T9_BASE_URL="$BASE_URL" npx playwright test; then
  echo "All T9 checks passed."
else
  fail "playwright facade checks"
fi
